...
contract AnonymousElection {
    ...
    function submitPK(
        ...
        require(round == 1, "Not in election round 1");
        require(canVote[msg.sender], "You are not allowed to vote");
        require(!hasSubmittedPK(msg.sender), "Already submitted public key");
    ...
    function vote(bytes calldata _candidate) public {
        require(round == 2, "Not in the voting round");
        require(canVote[msg.sender], "You are not allowed to vote");
        require(hasSubmittedVote(msg.sender) == false, "You already voted");
        uint256 check;
        ...
        require(check == 1, "Not the name of an candidate");
    ...
    // return generator g
    function getGeneratorG() public view returns (bytes memory) {
        return g;
    }
    ...
    function getPotentialCanidates() public view returns (string[] memory) {
        return candidates;
    }
    ...
    function getPKBytes() public view returns (bytes[] memory) {
        // Use the require function to make sure you are in a round >= 2
        require(
            round >= 2,
            "You need to be at least on round 2 of the election"
        );
        return allPKBytes;
    }
    ...
    function getVotes() public view returns (bytes[] memory) {
        ...
        require(
            round >= 3,
            "You need to be at least on round 3 of the election"
        );
        return allVotes;
    }
